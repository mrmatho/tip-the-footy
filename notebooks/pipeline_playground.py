...
def _(load_model_config, mo):
    ...
    get_last_clicks, set_last_clicks = mo.state(
        {
            "manual": 0,
            "baseline": 0,
            "conservative": 0,
    ...
    run_pipeline_btn = mo.ui.button(
        label="Run full pipeline",
        value=0,
        on_click=lambda value: (value or 0) + 1,
    )
    run_baseline_btn = mo.ui.button(
        label="Run Baseline preset",
        value=0,
        on_click=lambda value: (value or 0) + 1,
    )
    run_conservative_btn = mo.ui.button(
        label="Run Conservative Elo preset",
        value=0,
        on_click=lambda value: (value or 0) + 1,
    )
    run_aggressive_btn = mo.ui.button(
        label="Run Aggressive Elo preset",
        value=0,
        on_click=lambda value: (value or 0) + 1,
    )
...
def _(
    ...
    result_view = mo.md("Click **Run full pipeline** to execute with the selected settings.")
    ...
    clicks = {
        "manual": int(run_pipeline_btn.value or 0),
        "baseline": int(run_baseline_btn.value or 0),
        "conservative": int(run_conservative_btn.value or 0),
        "aggressive": int(run_aggressive_btn.value or 0),
    }
    last_clicks = get_last_clicks()

    triggered = [
        key for key in ("manual", "baseline", "conservative", "aggressive")
        if clicks[key] > int(last_clicks.get(key, 0))
    ]

    run_mode = "manual"
    selected_preset = None
    should_run = len(triggered) > 0
    ...
    if should_run:
        ...
        if int(start_year.value) > int(end_year.value):
            result_view = mo.md("Start year must be less than or equal to end year.")
        elif not (
        ...
        elif int(rolling_window_short.value) >= int(rolling_window.value):
            result_view = mo.md(
                "Use a short rolling window smaller than the long rolling window."
            )
        else:
            repo_root = Path.cwd()
    ...
    """.strip() + "\n"
            ...
            set_last_clicks(clicks)
    mo.vstack([result_view])
    return
